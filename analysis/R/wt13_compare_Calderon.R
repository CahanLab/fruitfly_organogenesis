...
TARGET_dir = file.path("results", ANALYSIS_VERSION, "compare_wt13_with_calderon")
dir.create(TARGET_dir)

seurat_data = readRDS("accessory_data/continuum_drosophila_embryonic_development_RNA/processed_data/continuum_exploration/14-16_celltyped.rds")
...
p<-ggplot(data=plot_df, aes(x=reorder(cell_types, proportion), y=proportion)) +
  ...
  xlab("Cell Types") + 
  ggtitle("stage 14-16: Calderon et al, 2022")
ggsave(filename = file.path(TARGET_dir, 'Calderon_cell_stg14-16_proportion.png'), plot = p, width = 14, height = 6)
...
saveRDS(combination_df, file = file.path(TARGET_dir, "spearman_correlation_wt13_Calderon.rds"))
