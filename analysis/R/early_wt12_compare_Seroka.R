...
TARGET_dir = file.path("results", ANALYSIS_VERSION, "compare_early_wt12_with_seroka")
...
p<-ggplot(data=plot_df, aes(x=reorder(cell_types, proportion), y=proportion)) +
  ...
  xlab("Cell Types") + 
  ggtitle("stage 12: Seroka et al, 2022")
ggsave(filename = file.path(TARGET_dir, 'Seroka_cell_stg12_proportion.png'), plot = p, width = 14, height = 6)
...
saveRDS(combination_df, file = file.path(TARGET_dir, 'spearman_correlation_early_wt12_Seroka.rds'))
...
p = ggplot(combination_df, aes(our_ct, other_ct, fill= spearman_correlation)) + 
  ...
  ggtitle("stage 10-12: Correlation similarity between our cell types and cell types from Seroka et al, 2022")
