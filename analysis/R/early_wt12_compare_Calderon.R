...
TARGET_dir = file.path("results", ANALYSIS_VERSION, "compare_early_wt12_with_calderon")
dir.create(TARGET_dir)

seurat_data = readRDS("accessory_data/continuum_drosophila_embryonic_development_RNA/processed_data/continuum_exploration/10-12_celltyped.rds")
seurat_data@meta.data$cell_type = stringr::str_trim(seurat_data@meta.data$cell_type, side = 'both')
...
p<-ggplot(data=plot_df, aes(x=reorder(cell_types, proportion), y=proportion)) +
  ...
  xlab("Cell Types") + 
  ggtitle("stage 10-12: Calderon et al, 2022")
ggsave(filename = file.path(TARGET_dir, 'Calderon_cell_stg10-12_proportion.png'), plot = p, width = 14, height = 6)

# look at spearman correlation between the two 
other_data = seurat_data
our_data = readRDS(file.path("results", ANALYSIS_VERSION, "manual_annotation_early_wt12/manual_celltype_object1.rds"))
...
saveRDS(combination_df, file = file.path(TARGET_dir, "spearman_correlation_early_wt12_Calderon.rds"))
...
p = ggplot(combination_df, aes(our_ct, other_ct, fill= spearman_correlation)) + 
  ...
  ggtitle("stage 10-12: Correlation similarity between our cell types and cell types from Calderon et al, 2022")
