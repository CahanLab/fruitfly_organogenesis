...
pdf(file = file.path(TARGET_dir, "early_dynamic_gene_heatmap.pdf"), height = 25)
...
scaled_exp = t(scale(t(smoothed_df)))
sorted_genes = names(sort(apply(scaled_exp, MARGIN = 1, FUN = which.max)))
pdf(file = file.path(TARGET_dir, "late_dynamic_gene_heatmap.pdf"), height = 25)
pheatmap(scaled_exp[sorted_genes, ], cluster_cols = FALSE, cluster_rows = FALSE)
dev.off()
...
pdf(file = file.path(TARGET_dir, "transient_dynamic_gene_heatmap.pdf"), height = 25)
pheatmap(scaled_exp[sorted_genes, ], cluster_cols = FALSE, cluster_rows = FALSE)
dev.off()
...
p <- ggplot(data=sub_biological_analysis, aes(x=reorder(Term, log_p), y=log_p)) +
  xlab("GO Biological Process") +
  ylab("-log10 adjusted P-value") +
  ggtitle("Transient Genes Enrichment") +
  geom_bar(stat="identity", fill="#00CED1") + coord_flip() + theme_bw()
ggsave(filename = file.path(TARGET_dir, "transient_go_enrichment.png"), plot = p, width = 10, height = 8)
