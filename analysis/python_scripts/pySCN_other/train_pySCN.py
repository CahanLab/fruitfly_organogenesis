...
st_df = pd.read_csv("raw_meta_tab.csv", index_col=0)
exp_df = exp_df.loc[:, st_df.index]
train_adata = sc.AnnData(exp_df.T)
train_adata.obs = st_df
#train_adata.X = exp_df.T.values

selected_cell_types = train_adata.obs['harmonized_celltypes'].value_counts().index[train_adata.obs['harmonized_celltypes'].value_counts() > 10]
selected_cell_types = np.array(selected_cell_types)
selected_cell_types = selected_cell_types[selected_cell_types != 'Unknown']

train_adata = train_adata[train_adata.obs['harmonized_celltypes'].isin(selected_cell_types), :]
expTrain, expVal = pySCN.splitCommonAnnData(train_adata, ncells=200, dLevel="harmonized_celltypes")
...
[cgenesA, xpairs, tspRF] = pySCN.scn_train(expTrain, nTopGenes = 75, nRand = 100, nTrees = 1500 ,nTopGenePairs = 75, dLevel = "harmonized_celltypes", stratify=True, limitToHVG=True, normalization = True)
...
assessment =  pySCN.assess_comm(expTrain, adVal, resolution = 0.005, nRand = 0, dLevelSID = None, classTrain = "harmonized_celltypes", classQuery = "harmonized_celltypes")
...
sub_expVal, other = pySCN.splitCommonAnnData(train_adata, ncells=70, dLevel="harmonized_celltypes")
